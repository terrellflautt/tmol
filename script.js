...
class TerminalAnimation {
    constructor() {
        ...
        // Randomly choose title between visits
        const titles = ['Lead DevOps', 'Cloud Architect'];
        const selectedTitle = this.getRandomTitle(titles);

        this.commands = [
            { command: 'whoami', output: selectedTitle },
    ...
    getRandomTitle(titles) {
        // Use visit count to alternate, but add some randomness
        const visitCount = parseInt(localStorage.getItem('visit_count') || '0');

        // Mix of deterministic (visit count) and random
        if (visitCount % 3 === 0) {
            // Every 3rd visit, random
            return titles[Math.floor(Math.random() * titles.length)];
        } else {
            // Otherwise alternate based on visit count
            return titles[visitCount % titles.length];
        }
    }
...
class ContactForm {
    ...
    handleEmailClick(e) {
        ...
        const accessKeys = {
            ...
            'snapitsoft@gmail.com': 'ebfa8f9b-ca95-40f0-abeb-23155a5a0c9c'
        };
...
class QuietDetails {
    ...
    beginUserJourney() {
        ...
        // Subtle confirmation then hide
        const journeyBtn = document.getElementById('begin-journey');
        journeyBtn.textContent = 'Journey Begun ✨';
        journeyBtn.style.background = 'rgba(102, 126, 234, 0.8)';
        journeyBtn.style.transform = 'scale(1.1)';

        setTimeout(() => {
            // Transform into tiny reset button and move to bottom-left corner
            journeyBtn.textContent = '↺';
            journeyBtn.title = 'Reset Journey (if you dare)';
            journeyBtn.style.cssText = `
                position: fixed;
                bottom: 10px;
                left: 10px;
                width: 24px;
            ...
            journeyBtn.addEventListener('mouseenter', () => {
                journeyBtn.style.opacity = '0.8';
                journeyBtn.style.transform = 'scale(1.2)';
            });
            ...
            // Change behavior to reset journey on click
            const newBtn = journeyBtn.cloneNode(true);
            journeyBtn.parentNode.replaceChild(newBtn, journeyBtn);

            newBtn.addEventListener('click', () => {
                if (confirm('Reset your entire journey? All discoveries will be preserved, but you\'ll start fresh.')) {
                    localStorage.removeItem('journey_begun');
                    location.reload();
                }
            });
        }, 2000);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (visitCount % 3 === 0) {
        ...
        selectedTitle = titles[visitCount % titles.length];
    }

    document.title = `Terrell K. Flautt | ${selectedTitle}`;
