...
👻 Or stay anonymous - your magic is preserved either way.
    ...
    getUserGoalProfile() {
        // Generate user goal profile for AI personalization
        const goalProfile = {
            visitReason: localStorage.getItem('visit_reason') || 'exploration',
            interests: this.extractUserInterests(),
            goals: this.extractUserGoals(),
            engagement: this.calculateEngagementLevel(),
            preferences: {
                technical: this.hasInteractedWith('code') || this.hasInteractedWith('projects'),
                creative: this.hasInteractedWith('design') || this.hasInteractedWith('logo'),
                professional: this.hasInteractedWith('resume') || this.hasInteractedWith('contact')
            }
        };
    ...
    extractUserInterests() {
        ...
        interactions.forEach(interaction => {
            if (interaction.type === 'click' && interaction.target) {
                if (interaction.target.includes('project')) interests.add('projects');
                if (interaction.target.includes('blog')) interests.add('blog');
                if (interaction.target.includes('contact')) interests.add('contact');
            }
        });
    ...
    extractUserGoals() {
        // Infer user goals from navigation patterns
        const goals = [];
        const pageViews = JSON.parse(localStorage.getItem('page_views') || '[]');

        if (pageViews.includes('projects')) goals.push('view_portfolio');
        if (pageViews.includes('contact')) goals.push('get_in_touch');
        if (pageViews.includes('blog')) goals.push('learn_more');
    ...
    calculateEngagementLevel() {
        ...
        const discoveries = parseInt(localStorage.getItem('discovery_count') || '0');
        const timeSpent = parseInt(localStorage.getItem('total_time_spent') || '0');

        if (visits > 5 && discoveries > 3) return 'high';
        if (visits > 2 || discoveries > 1) return 'medium';
        return 'low';
    }

    hasInteractedWith(keyword) {
        const interactions = JSON.parse(localStorage.getItem('user_interactions') || '[]');
        return interactions.some(i => i.target && i.target.toLowerCase().includes(keyword));
    }
    ...
    analyzeExplorationPattern() {
        ...
        const patterns = {
            methodical: 0,  // Systematic, linear exploration
            curious: 0,     // Random, explorative clicking
            focused: 0,     // Direct navigation to specific sections
            passive: 0      // Minimal interaction
        };

        // Analyze click patterns
        if (interactions.length > 20) patterns.methodical += 3;
        if (interactions.length > 10 && visits < 3) patterns.curious += 3;
        if (discoveries.length > 5) patterns.curious += 2;
        if (interactions.length < 5 && visits > 1) patterns.passive += 3;
        ...
        return {
            primaryStyle: maxPattern,
            patterns: patterns,
            interactionCount: interactions.length,
            discoveryRate: discoveries.length / Math.max(visits, 1)
        };
    }
