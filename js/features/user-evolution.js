...
class ReturningVisitorEvolution {
    ...
    activateNewcomerFeatures() {
        // Gentle guidance for newcomers - removed logo hint due to conflict with Aziza encounter
        // Logo triple-click already triggers Aziza introduction (master story system)
    }
    ...
    unlockStreakReward(reward) {
        // Handle streak-based rewards (similar to milestone rewards)
        this.showRewardUnlock(reward);
        this.activateReward(reward.activation);

        // Log streak achievement
        console.debug('🔥 Streak reward unlocked:', reward.reward);

        // Save streak reward state
        const streakRewards = JSON.parse(localStorage.getItem('terrellflautt_streak_rewards') || '[]');
        streakRewards.push({
            reward: reward.reward,
            unlockedAt: Date.now(),
            streak: this.calculateVisitStreak()
        });
        localStorage.setItem('terrellflautt_streak_rewards', JSON.stringify(streakRewards));

        // Trigger special streak effects
        if (window.visualMusicExperience) {
            window.visualMusicExperience.triggerForMajorAchievement({
    ...
    showRewardUnlock(reward) {
        ...
        // Handle both object and string reward formats
        const rewardName = typeof reward === 'string' ? reward : (reward.reward || 'Reward');
        const rewardDesc = typeof reward === 'string' ? 'Achievement unlocked!' : (reward.description || '');

        rewardNotification.innerHTML = `
            <div class="reward-content">
                <div class="reward-icon">🏆</div>
                <h3>Milestone Achieved!</h3>
                <div class="reward-name">${String(rewardName).replace(/_/g, ' ').toUpperCase()}</div>
                <div class="reward-description">${rewardDesc}</div>
