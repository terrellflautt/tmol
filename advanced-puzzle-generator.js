...
class AdvancedPuzzleGenerator {
    ...
    setupPuzzleGeneration() {
        // Initialize puzzle generation system
        console.debug('Puzzle generation system initialized');
    }
    ...
    setupPuzzleEventListeners(puzzleInterface) {
        const generateBtn = puzzleInterface.querySelector('.generate-puzzle-btn');
        const dailyBtn = puzzleInterface.querySelector('.daily-challenge-btn');
        const minimizeBtn = puzzleInterface.querySelector('.minimize-puzzle-btn');

        generateBtn.addEventListener('click', () => {
            const puzzle = this.generateAdaptivePuzzle();
            this.displayPuzzle(puzzle, puzzleInterface);
        });

        dailyBtn.addEventListener('click', () => {
            const dailyChallenge = this.createDailyChallenge();
            this.displayPuzzle(dailyChallenge, puzzleInterface);
        });

        minimizeBtn.addEventListener('click', () => {
            puzzleInterface.style.transform = 'translateY(calc(100% - 60px))';
            setTimeout(() => {
                puzzleInterface.style.transform = 'translateY(0)';
    ...
    displayPuzzle(puzzle, puzzleInterface) {
        const content = puzzleInterface.querySelector('.puzzle-content');
